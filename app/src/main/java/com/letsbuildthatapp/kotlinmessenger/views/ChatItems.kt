...
class ChatFromItem(val text: String, val user: User, val onClick: (User) -> Unit): Item<ViewHolder>() {
  override fun bind(viewHolder: ViewHolder, position: Int) {
    ...
    viewHolder.itemView.imageview_chat_from_row.setOnClickListener {
      onClick(user)
    }
  }
...
class ChatToItem(val text: String, val user: User, val onClick: (User) -> Unit): Item<ViewHolder>() {
  override fun bind(viewHolder: ViewHolder, position: Int) {
